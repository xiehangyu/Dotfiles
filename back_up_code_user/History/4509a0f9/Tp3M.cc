...
inline double D2distance(int i, int j,int L)
{
    int x1=i%L;
    int y1=i/L;
    int x2=j%L;
...
{//Time evolution using TDVP, the decay exponent of the interaction is alpha, the time step is t, the total time is tend, the number of global subspace expansion is n_GSE, the order of Krylov subspace is Korder, the two printed results are the deviation of the expectation value of the total spin from its maximum value and the half chain entropy.
    ofstream fp,gp;
    vector<MPS> psis;
    fp.open("./data/NewOBCEntropyresultN"+to_string(N)+"alpha"+to_string(alpha)+"t"+to_string(t)+"tend"+to_string(tend)+"GSE"+to_string(n_GSE)+"Korder"+to_string(Korder)+"randomin"+to_string(randomin)+"J"+to_string(J)+".txt");
    gp.open("./data/OBCOverlapN"+to_string(N)+"alpha"+to_string(alpha)+"t"+to_string(t)+"tend"+to_string(tend)+"GSE"+to_string(n_GSE)+"Korder"+to_string(Korder)+"randomin"+to_string(randomin)+"J"+to_string(J)+".csv");
...
{//The input parameters are alpha, number of sites, time step, total time, number of global subspace expansion, order of Krylov subspace
	...
	double h=stod(argv[8]);
        overlap(alpha,t,tend,GSE,Korder,J,h);
    }
    return 0;
}
