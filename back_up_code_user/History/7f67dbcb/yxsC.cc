...
{//Construct the long-range Hamiltonian with exponent alpha
    ...
    for(int i=1;i<=N;i++)
    {
        for(int j=i+1;j<=N;j++)
        {
            ampo+=4*normfactor/pow(1.0+Period_length(i,j),alpha),"Sx",i,"Sx",j;
        }
        ampo += J*2,"Sz",i;
    }
    auto H=toMPO(ampo);
...
{//Time evolution using TDVP, the decay exponent of the interaction is alpha, the time step is t, the total time is tend, the number of global subspace expansion is n_GSE, the order of Krylov subspace is Korder, the two printed results are the deviation of the expectation value of the total spin from its maximum value and the half chain entropy.
    ofstream fp;
    vector<MPS> psis;
    fp.open("./data/OBCEntropyresultN"+to_string(N)+"alpha"+to_string(alpha)+"t"+to_string(t)+"tend"+to_string(tend)+"GSE"+to_string(n_GSE)+"Korder"+to_string(Korder)+"randomin"+to_string(randomin)+"J"+to_string(J)+".txt");
...
addBasis(psi1,H,epsilonK,{"Cutoff",1E-6,"Method","DensityMatrix","KrylovOrd",Korder,"DoNormalize",true,"NumCenter",1});
 //addBasis(psi1,H,epsilonK,{"Cutoff",1E-6,"Method","DensityMatrix","KrylovOrd",Korder,"DoNormalize",true});
    ...
    fp.close(); 
    fp.open("./data/OBCoverlapN"+to_string(N)+"alpha"+to_string(alpha)+"t"+to_string(t)+"tend"+to_string(tend)+"GSE"+to_string(n_GSE)+"Korder"+to_string(Korder)+"randomin"+to_string(randomin)+"J"+to_string(J)+".csv");
    int total_length=psis.size();
    for(int i=0;i<total_length;i++)
    {
        for(int j =0;j<total_length;j++)
        {
            auto overlap_value=innerC(psis[i],psis[j]);
            fp<<real(overlap_value)<<","<<imag(overlap_value);
            if(j<total_length-1)
            {
                fp<<",";
            }
