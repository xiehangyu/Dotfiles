...
def create_W_matrix4(alpha,beta):
    W=np.array([[1,0,0,np.sqrt(alpha*beta)],[0,alpha,np.sqrt(beta*alpha),0],[0,np.sqrt(beta*alpha),beta,0],[np.sqrt(alpha*beta),0,0,alpha*beta]])
    W=np.sqrt(W)
    return(W)
...
def test_phase_transition2Dplane(coefs):
    ...
    for coef in coefs:
        ...
        W=create_W_matrix4(alpha,beta)
        m1ss=np.sort(np.abs(scipy.linalg.eigvals(diagonal_symmetry(W,Nv))))[::-1]
        m2ss=np.sort(np.abs(scipy.linalg.eigvals(diagonal_asymmetry(W,Nv))))[::-1]
        m3ss=np.sort(np.abs(scipy.linalg.eigvals(offdiagonal_symmetry(W,Nv))))[::-1]
        m4ss=np.sort(np.abs(scipy.linalg.eigvals(offdiagonal_asymmetry(W,Nv))))[::-1]
        m1=m1ss[0]
        m2=m2ss[0]
        m3=m3ss[0]
        m4=m4ss[0]
        print("The coeficient is {}".format(coef))
        print(m1ss[:20])
        print(m2ss[:20])
        print(m3ss[:20])
        print(m4ss[:20])
...
def test_alphabeta_gap():
    ...
    for coef in coefs:
        ...
        if coef<=1/2:
            theos.append(2*coef*(1+(2*coef-1)**(Nv-1)))
        else:
            theos.append(2*(1-coef)*(1-(2*coef-1)**(Nv-1)))
...
def degeneracy_of_transfer(coefs):
    ...
    eigenplots=[]
    for coef in coefs:
        W=create_W_matrix1(coef)
        loop_transfer=diagonal_symmetry(W,Nv)
        loop_transfer2=offdiagonal_phi_loop(W,Nv)
        eigs=np.sort(np.abs(np.linalg.eigvals(loop_transfer)))[::-1][:32]
...
if __name__=='__main__':
    ...
    coefs=np.linspace(0.63,0.66,20)
