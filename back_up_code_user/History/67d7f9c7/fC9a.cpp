...
void interpolationexpression(vector<int> & xs, vector<double> & fs, vector<double> &M, bool whether_add)
{ //calculate the coefficients of the cubic spline, if whether_add is true, we open the file with ios::app
    double a0,a1,a2,a3;
    ofstream fp;//store the coefficients of the cubic spline for later drawing.
    vector<double>h;
    int n=xs.size()-1;
    h.resize(n);
    if(whether_add==false)
        fp.open("./data/interpolationexpression.txt");
    else
        fp.open("./data/interpolationexpression.txt",ios::app);
    if(!fp.is_open())
    {
        cout<<"open file failed"<<endl;
        return;
...
int main()
{
    ...
    readdata(xs,fs);
    spline(xs,fs,M);
    interpolationexpression(xs,fs,M,false);
