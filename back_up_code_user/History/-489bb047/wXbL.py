...
def page_curve_comparison_hamiltonian_evolution(initial_time,spacing=2):
    ...
    entropy_array2=[]
    fs=open("./plot_data/OnSitePotential_Hamiltonian_evolution_Nsite{}_Tperiod{}_Initialtime{}_Samplingtime{}Spacing{}Period1densitywave.txt".format(N_sites,T_period,initial_time,Sampling_time,spacing),'w')
    for j in range(len(calculate_list)):
        fs.write("{}\t{}\n".format(calculate_list[j],entropy_array[j]))
