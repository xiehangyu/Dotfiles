...
def Transverse_Ising_XX(N,h=0.0):
    Sx=jmat(N/2,'x')
    Sz=jmat(N/2,'z')
    return 1.0/N*Sz*Sz*4+h*Sx*2

def Transverse_Ising(N,h=0.0):
    Sx=jmat(N/2,'x')
    Sz=jmat(N/2,'z')
    return 1.0/N*Sx*Sx*4+h*Sz*2
...
def initialstate_z(N):
    state=basis(N+1,0)
    return state.unit()

def initialstate_x(N):
    state=Qobj(dims=[[N+1],[1]])
    for i in range(0,N+1):
        state += basis(N+1,i)*np.sqrt(ssc.comb(N,i)/2**N)
    state=state.unit()
...
def drawing_entropy_1115(c_str,t,tend,N,h):
    initialstate=initialstate_z(N)
    filename="/home/xiehangyu/yxh/study/mpq/Long_range_area_law/procedure/TDVP/sample/data/"
    if c_str=='x':
        H=Transverse_Ising_XX(N,h)
        filename+="XXexact"
    else:
        H=Transverse_Ising(N,h)
        filename+="exact"
    times=np.linspace(0,t,tend)
    results=mesolve(H,initialstate,times,[],[])
    filename+="N_{}t_{}tend_{}h_{}.txt".format(N,t,tend,h)
    fp=open(filename,'w')
    for i in range(len(times)):
        ...
        NAdensity=splitstate.ptrace(0)
        fp.write(str(times[i])+" "+str(entropy_vn(NAdensity,base=2))+"\n")
    fp.close()
    ...
    for c_str in ['x','z']:
        for N in [50,100,200,400,800]:
            for h in [0.2,0.5,0.8]:
                drawing_entropy_1115(c_str,t,tend,N,h)
...
def NearestNeighborOBC(N,J=0.0):
    X=sigmax()
    Z=sigmaz()
...
def simple_longrange_HamiltonianOBC(N,alpha,J=0.0):
    Z=sigmaz()/2
    X=sigmax()/2
    return_H=Qobj(dims=[[2]*N,[2]*N])
    for i in range(N):
        for j in range(i+1,N):
            return_H += tensor([identity(2)]*i+[X]+[identity(2)]*(j-i-1)+[X]+[identity(2)]*(N-j-1))/(np.abs(i-j))**alpha
    for i in range(N):
        return_H += J*tensor([identity(2)]*i+[Z]+[identity(2)]*(N-i-1))
    return return_H/N**(1-alpha)
...
def rank_of_states(results,cutoff=1E-7,if_drawing=True):
    times=results.times
    states=results.states
    ranks=[]
    for i in range(len(times)):
...
def trial_eigenvalues(N,alpha=0.1,if_drawing=False):
    ...
    largesteigenvalues(rho,N,alpha,if_drawing)
    overlapstates_totalangularoperator(rho,N,alpha,if_drawing)
    cutoff=1E-7
    ranks=rank_of_states(results,cutoff,if_drawing)
    fp=open("./plot_data/rank_N_{}_alpha_{}_cutoff_{}.txt".format(N,alpha,cutoff),'w')
    for i in range(len(ranks)):
        fp.write("{}  {}\n".format(results.times[i],ranks[i]))
    fp.close()
