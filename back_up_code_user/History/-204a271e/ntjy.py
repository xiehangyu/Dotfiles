...
def addtrianglewithoutbound(tensorABC):
    '''
    B           B
        C to       triangle   C
    ...
    u,s,v=LA.svd(tensorABC.reshape([dA,dB*dC]))
    print(s)
    cutoff1=min(int((dB*dC)),int((dA)))
    tensorA=u[:,:cutoff1]
    tensorBC=(np.diag(s[:cutoff1])@v[:cutoff1,:]).reshape([cutoff1,dB,dC])
    sqrtc=int(np.sqrt(cutoff1))
    for split1 in range(sqrtc,0,-1):
        if cutoff1%split1==0:
            break
    split2=cutoff1//split1
    U=unitary_group.rvs(cutoff1).reshape([cutoff1,split1,split2])
    tensorA=ncon([tensorA,U.conj()],[[-1,1],[1,-2,-3]])
    tensorBC=ncon([U,tensorBC],[[1,-1,-2],[1,-3,-4]])
    u,s,v=LA.svd(tensorBC.transpose([0,2,1,3]).reshape([split1*dB,split2*dC]))
    print(s)
    cutoff2=min([split1*dB,split2*dC])
    tensorB=(u[:,:cutoff2]@np.diag(s[:cutoff2])).reshape([split1,dB,cutoff2]).transpose([1,0,2])
    tensorC=v[:cutoff2,:].reshape([cutoff2,split2,dC]).transpose([2,0,1])
    return tensorA, tensorB, tensorC


def Moses_Move_withoutbound(C_arrays,chi,D):
    Lx=len(C_arrays)
    bottom_lay_C_arrays=[]
    Upper_lay_C_arrays=[]
    tensorA,tensorB,tensorC=addtrianglewithoutbound(C_arrays[0].transpose([0,1,2,4,3]).reshape(D*chi**2,chi,chi))
    bottom_lay_C_arrays.append(tensorA.reshape([D,chi,chi,tensorA.shape[1],tensorA.shape[2]]).transpose([0,1,2,4,3]))
    Upper_lay_C_arrays.append(tensorB.transpose([1,2,0]))
    newtensor=(ncon([tensorC,C_arrays[1]],[[1,-4,-2],[-1,1,-3,-6,-5]])).reshape([D*chi*tensorC.shape[2],chi*tensorC.shape[1],chi])
    for i in range(1,Lx-1):
        tensorA,tensorB,tensorC=addtrianglewithoutbound(newtensor)
        bottom_lay_C_arrays.append(tensorA.reshape([D,tensorA.shape[0]//D//chi,chi,tensorA.shape[1],tensorA.shape[2]]).transpose([0,1,2,4,3]))
        Upper_lay_C_arrays.append(tensorB.reshape([tensorB.shape[0]//chi,chi,tensorB.shape[1],tensorB.shape[2]]).transpose([0,2,3,1]))
        newtensor=(ncon([tensorC,C_arrays[i+1]],[[1,-4,-2],[-1,1,-3,-6,-5]])).reshape([D*chi*tensorC.shape[2],chi*tensorC.shape[1],chi])
    u,s,v=LA.svd(newtensor.transpose([0,2,1]).reshape([newtensor.shape[0]*newtensor.shape[2],newtensor.shape[1]]))
    bottom_lay_C_arrays.append((u[:,:s.shape[0]]).reshape([D,newtensor.shape[0]//D//chi,chi,chi,s.shape[0]]))
    Upper_lay_C_arrays.append((np.diag(s)@v[:s.shape[0],:]).reshape([s.shape[0],newtensor.shape[1]//chi,chi]).transpose([1,0,2]))
...
def test_Moses_Move_without_bound(C_arrays,chi,D):
    ...
    bottom_lay_C_arrays,Upper_lay_C_arrays=Moses_Move_withoutbound(C_arrays,chi,D)
    ...
    difference_norm=LA.norm(original_tensor-new_tensor)
...
if __name__=='__main__':
    ...
    bottom_lay,upper_lay=test_Moses_Move_without_bound(C_arrays,chi,D)
