...
def MPS_fixedpoint_contraction(fold_o1,fold_o2,x2,y2,fold_c,chi,eigenstate=None):
    ...
    if eigenstate is None:
        ...
        eigenstate=eigenstate/np.vdot(kron_product([(np.eye(chi)/np.sqrt(chi)).reshape([chi**2])]*Lx),eigenstate)
    eigenstate=eigenstate.reshape([bd]*Lx)
    fold_in=folded_identity(chi)
    #print(ncon([fold_in]*Lx+[eigenstate],[[i] for i in range(1,Lx+1)]+[[i for i in range(1,Lx+1)]]))
    #htm=horizontal_transform_matrix(fold_c,Lx,chi)
    #reference_vector=ncon([htm,eigenstate],[[j for j in range(1,Lx+1)]+[-Lx+j for j in range(Lx)],[j for j in range(1,Lx+1)]])
    #print(LA.norm(reference_vector-eigenstate))
    ...
    if x2>0:
        ...
        if y2==0:
            ...
            return ncon([start_c,eigenstate],[[i for i in range(1,Lx+1)],[j for j in range(1,Lx+1)]])
        start_c=ncon([start_c,fold_c,fold_in],[[-j for j in range(1,x2+1)]+[1],[1,-x2-1,2,-x2-2],[2]])
        start_c=ncon([start_c,eigenstate],[[i for i in range(1,Lx+1)]+[-1],[j for j in range(1,Lx+1)]])
        for i in range(y2-1):
...
def MPS_contraction_general_with_fixed_point(fold_o1,fold_o2,x1,y1,x2,y2,fold_c,chi,eigenstate=None):
    #Here we assume y2>y1
    if y1==0:
        return MPS_direct_contraction(fold_o1,fold_o2,x2-x1,y2-y1,fold_c,chi)
    if y1>0:
        return MPS_fixedpoint_contraction(fold_o1,fold_o2,x2-x1,y2-y1,fold_c,chi,eigenstate)

def MPS_contraction_general_with_fixed_point_from_op(O1,O2,x1,x2,y1,y2,C_array,D,chi,eigenstate=None):
    #Here we assume y2>y1
    fold_o1=folded_operator_tensor(C_array,O1,D,chi)
    fold_o2=folded_operator_tensor(C_array,O2,D,chi)
    fold_c=foldedchannel(C_array,D,chi)
    return MPS_contraction_general_with_fixed_point(fold_o1,fold_o2,x1,y1,x2,y2,fold_c,chi,eigenstate)
...
def construct_heatmap(Lx,D,chi):
    fold_o1=np.random.random([chi**2]*4)+1j*np.random.random([chi**2]*4)
    fold_o2=np.random.random([chi**2]*4)+1j*np.random.random([chi**2]*4)
    from decomposition import random_dual_PEPS_fold
    fold_c=random_dual_PEPS_fold(D,chi)
    ...
    O1=O1-np.trace(O1)*np.eye(D)/D
    O2=O2-np.trace(O2)*np.eye(D)/D
    fold_o1=folded_operator_tensor(fold_c,O1,D,chi)
    fold_o2=folded_operator_tensor(fold_c,O2,D,chi)
    C_array=classical_partition_function_C_array()
    fold_c=foldedchannel(C_array,D,chi)
    '''
    x1=0
    M_matrix=np.zeros([Lx-2,Lx])
    for i in range(0,Lx):
        Lx_temp=i+1
        htm=horizontal_transform_matrix(fold_c,Lx_temp,chi)
        bd=chi**2
        htm=htm.transpose([j for j in range(Lx_temp)]+[2*Lx_temp-j-1 for j in range(Lx_temp)])
        htm=htm.reshape([bd**Lx_temp]*2).transpose()
        eigenvalues,eigenstates=scipy.linalg.eig(htm)
        idx=eigenvalues.argsort()[::-1]
        eigenvalues=eigenvalues[idx]
        eigenstates=eigenstates[:,idx]
        print(eigenvalues[:5])
        eigenstate=eigenstates[:,0]
        eigenstate=eigenstate/np.vdot(kron_product([(np.eye(chi)/np.sqrt(chi)).reshape([chi**2])]*Lx_temp),eigenstate)
        x2=i
        for j in range(1,Lx-2):
            y1=j
            y2=y1+2
            temp1=MPS_contraction_general_with_fixed_point(fold_o1,fold_o2,x1,y1,x2,y2,fold_c,chi,eigenstate)
            temp2=PEPS_direct_contraction(fold_o1,fold_o2,x1,y1,x2,y2,fold_c,chi,Lx,Lx)
            M_matrix[j,i]=np.log10(abs(temp1-temp2)/abs(temp2))
    plt.imshow(M_matrix, cmap='hot', interpolation='nearest',origin='lower')
    plt.colorbar()
    plt.show()
