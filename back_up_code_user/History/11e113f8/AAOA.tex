...
the superfluid density is defined as $n_s:=\delta\bm{j}_s/\delta\bm{v}_s$, determined by the coefficient before the quadratic term of the superfluid velocity $\bm{v}_s$ in the effective action \eqref{eq:effective}. From the effective action and the assumption that $\bm{v}_{+}=-\bm{v}_{-}=\bm{v}_{s}$,
...
One can experimentally measure this complex spectrum from the spectral function, which is defined as 
\begin{equation}
