...
auto LongrangeHamiltonian(double alpha,double h)
{//Construct the long-range Hamiltonian with exponent alpha
    auto ampo=AutoMPO(sites);
    double normfactor=1.0/max(pow(N,1-alpha),1.0);
    for(int i=1;i<=N;i++)
    {
        for(int j=i+1;j<=N;j++)
        {
...
void timeevolutionTDVP(double alpha, double t=0.1, double tend=0.5, int n_GSE=3, int Korder=3, double randomin=0,double h=0)
{//Time evolution using TDVP, the decay exponent of the interaction is alpha, the time step is t, the total time is tend, the number of global subspace expansion is n_GSE, the order of Krylov subspace is Korder, the two printed results are the deviation of the expectation value of the total spin from its maximum value and the half chain entropy.
    ofstream fp;
    fp.open("./data/EntropyresultN"+to_string(N)+"alpha"+to_string(alpha)+"t"+to_string(t)+"tend"+to_string(tend)+"GSE"+to_string(n_GSE)+"Korder"+to_string(Korder)+"randomin"+to_string(randomin)+"J"+to_string(h)+".txt");
    auto psi1=productstate();
    auto H=LongrangeHamiltonian(alpha,h);
    ...
    for(int n = 1; n <= nsw; ++n)
        ...
        if(n < n_GSE)
            ...
            {
                epsilonK.push_back(1E-10);
            }
            
addBasis(psi1,H,epsilonK,{"Cutoff",1E-6,"Method","DensityMatrix","KrylovOrd",Korder,"DoNormalize",true,"NumCenter",2});
 //addBasis(psi1,H,epsilonK,{"Cutoff",1E-6,"Method","DensityMatrix","KrylovOrd",Korder,"DoNormalize",true});
            }
        
        // TDVP sweep
        if(t*n<=20){
            tdvp(psi1,H,-t*1_i,sweeps,{"Truncate",true,"DoNormalize",true,"Quiet",true,"ErrGoal",1E-10,"IsHermitian",true,"NumCenter",2});}
        else{
...
{//The input parameters are alpha, number of sites, time step, total time, number of global subspace expansion, order of Krylov subspace, random potential, strength or interaction.
