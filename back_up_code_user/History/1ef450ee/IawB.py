...
from scipy import stats.unitary_group
...
def channel_tangent_dimension(c_array,D,d):
    ...
    print("The tangenet dimension for channel is {}".format(2*D**2*d-rank))

def unital_tangent_dimension(c_array,D,d):
    CM=unital_condition(c_array,D,d)
    rank=np.linalg.matrix_rank(CM)
    print("The tangenet dimension unital channel is {}".format(2*D**2*d-rank))
...
def constructmethod1(D,d):
    c_array=np.zeros([d,D,D],dtype=np.complex128)
    c_array[0,:,:]=unitary_group.rvs(D)
    total_check(c_array,D,d)
    total_check_dimension(c_array,D,d)

def constructmethod2():
    p1=np.random.random()
    p2=np.random.random()
    c_array=np.zeros([2,2,2],dtype=np.complex128)
    c_array[0,0,0]=np.sqrt(p1)
    c_array[0,1,1]=np.sqrt(p2)
    c_array[1,0,0]=np.sqrt(1-p1)
    c_array[1,1,1]=np.sqrt(1-p2)
    total_check(c_array,2,2)
    total_check_dimension(c_array,2,2)