...
def Naive_generation_go_through(D):
    U=unitary_group.rvs(D**2).reshape([D,D,D,D])
...
def calculate_tangent_space_dimension_go_through(C_array,D,chi):
    ...
    for i, func in enumerate(functions):
        def wrapped_func(x):
            return func(x,D,chi)
        M[k,:] = approx_fprime(point, wrapped_func, epsilon)
...
def go_through_tangent_space_naive_case(D):
    ...
    calculate_tangent_space_dimension_go_through(C_array,D,D)
...
def Go_through_guess_by_minimization(D,chi):
    ...
    C_array_temp=result.x.reshape([2,D,chi,chi,chi,chi])
    C_array=C_array_temp[0]+1j*C_array_temp[1]
    calculate_tangent_space_dimension_go_through(C_array,D,chi)