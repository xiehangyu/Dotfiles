...
def test_eigenvalues_entropy(fold_c,Lx,chi):
    htm=horizontal_transfer_matrix(fold_c,Lx,chi)
    htm=htm.transpose([j for j in range(Lx)]+[2*Lx-j-1 for j in range(Lx)])
    htm=htm.reshape([chi**Lx]*2)
    ...
    dx=chi**(Lx//2)
    eigenstateM=eigenstate.reshape([dx,chi**(Lx-Lx//2)])
...
if __name__=='__main__':
    ...
    print(test_eigenvalues_entropy(fold_c,6,2))