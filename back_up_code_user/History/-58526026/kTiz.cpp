...
void generating_majoranamatrix(MatrixXcd &Majorana_matrix, int n)
{
    ...
    for(int i=0;i<=L-1;i++)
    {
        for(int j=0;j<=L-1;j++)
        {
            ...
            Majorana_matrix(2*i+1,2*j+1)=varphi_j_map[j-i];
        }
    }
}
void verify_covariance_scaling(int n)
{
    ...
    ofstream fp;
    string filename;
    filename="./plot_data/"+file_name+"covariance_scaling n="+to_string(n)+".txt";
    fp.open(filename);
    int old_L=L;
    if(L>20000)
    {
...
MatrixXcd normalized_gamma_matrix(MatrixXcd &gamma_matrix)
{
    ...
    return complex<double>(0.0,-1.0) * eigenvectors * normalized_matrix * eigenvectors.adjoint();
}
template <typename T>
...
void Majorana_hamiltonian(int n)
{
    ...
    file_name=file_name+"Majorana"+to_string(n);
    MatrixXcd H;
...
void Majorana_gamma(int n)
{
    ...
    L=L*2;
    file_name=file_name+"Majoranagamma"+to_string(n);
...
{/*
  ...
  /*  cout << "the alpha is:\n";
    ...
    Nsites = pow(L, D);
    x_floquet = 0.15;
    lambda_floquet = 1/2.0*asinh(cos(2*x_floquet)*cos(2*x_floquet)/sin(2*x_floquet))+stod(argv[2]); 
    //lambda_floquet = 0.8;
