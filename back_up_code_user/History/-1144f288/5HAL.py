...
def diagonal_Greenfunction_with_gamma(epsilon,omega,UR):
    ...
    for gamma in np.linspace(0.5,1.5,100):
        diags.append(np.diag(GreenFunction(UR,gamma,epsilon,omega)))
        gammas.append(gamma)
    plt.plot(gammas,diags)
    plt.legend(["1","2","3","4"])
    plt.show()

def diagonal_Greenfunction_with_epsilon(omega,UR,gamma):
    diags=[]
    epsilons=[]
    for epsilon in np.linspace(0,3.5,500):
        diags.append(np.diag(GreenFunction(UR,gamma,epsilon,omega)))
        epsilons.append(epsilon)
    plt.plot(epsilons,diags)
    plt.legend(["1","2","3","4"])
    plt.show()