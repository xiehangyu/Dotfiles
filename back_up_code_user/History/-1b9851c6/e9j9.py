...
def state5optimization1D(thetas,L,alpha):
    ...
    for k in range(L):
        ...
        if k%5==0:
            ...
            sumans += 1/(PD(i,k,L)**alpha)
    return sumans/5.0*2.0


def minimizationstate5optimization1D():
    L=int(sys.argv[1])
    initguess=np.random.random([8])*2*np.pi
    print("The initguess is {}".format(initguess))
    alpha=float(sys.argv[2])
    print(L,alpha)
    def state5optimization1Dwrapper(thetas):
        return state5optimization1D(thetas,L,alpha)
    res=minimize(state5optimization1Dwrapper,initguess,method='BFGS',)
    print("最小化的角度是{}".format(res.x))
    print("最小化的值是{}".format(res.fun))
    print(res)


def D3integratetrial(L,alpha):
    lengths=[]
    results=[]
    for i in range(1,L+1):
        ...
        for y in range(-i,i+1):
            for z in range(-i,i+1):
                result_sum+=(-1)**(x+y+z-1)/(x**2+y**2+z**2)**(alpha/2)
        x=-i
        for y in range(-i,i+1):
            for z in range(-i,i+1):
                result_sum+=(-1)**(x+y+z-1)/(x**2+y**2+z**2)**(alpha/2)
    ...
    for j in results:
        if j!=0:
            results2.append(np.log(np.abs(j)))
        else:
            results2.append(-100)
    lengths2=[np.log(j) for j in lengths]
    plt.plot(lengths2,results2)
    plt.show()
    #Fitting legnths2 and results2
    from scipy.optimize import curve_fit
    def func(x,a,b):
        return a*x+b
    popt,pcov=curve_fit(func,lengths2,results2)
    print(popt)
    print(pcov)
    lengths3=lengths2[40:]
    results3=results2[40:]
    popt,pcov=curve_fit(func,lengths3,results3)
    print(popt)
    print(pcov)
    plt.plot(lengths3,results3)
...
if __name__=='__main__':
   ...
   minimizationstate5optimization1D()
