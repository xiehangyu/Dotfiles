...
void JacobyEig(vector<vector<double>> & M,vector<vector<double>> &  Q,vector<double> & eigenvalue)
{
...
void createMatrix(vector<vector<double>> & M, vector<int> & label, vector<vector<double>> & original_matrix)
{
    ...
    char c;
    M.resize(4);
    original_matrix.resize(end);
    for(int i=0;i<=end-1;i++)
    {
        original_matrix[i].resize(4);
    }
    label.resize(150);
    for(int i=0;i<=3;i++)
    {
...
void PCA()
{
    ...
    createMatrix(M,label,original_matrix);
    ...
    MT=transpose_matrix(original_matrix);
    ofstream fp;
    ...
    {  
        for(int j=0;j<=1;j++)
        {
           // cout<<inner_product(projection_matrix[j],MT[i])<<"\t";
            fp<<inner_product(projection_matrix[j],[i])<<"\t";
        }
      //  cout<<label[i]<<endl;
        fp<<label[i]<<endl;
    }
