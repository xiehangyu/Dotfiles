...
BCS model \cite{Yamamoto2019,footnote1} 
\begin{equation}
...
\end{equation}
For a general phase transition with spontaneous symmetry breaking and order parameter $\Delta_0$ with the dimension of energy, we have $\Delta E\propto\chi^2$ \cite{footnote2}. The condensation energy is thus directly related to the number of roots $\chi$. 
...
\end{equation}
Since we only consider the case with a large $\beta$, we directly substitute Eq. (\ref{Delta_0}) into the dispersion relation \cite{footnote3}. For the points not on the phase boundary, we have $\text{Re}
