...
class CustomNormalize(Normalize):
    def __init__(self,threshold=0.13,vmin=None, vmax=None, clip=None):
...
# Plotting
    ...
    norm=CustomNormalize(threshold=thresheld)
    plt.figsize=(10,5)
    fp=open(filename,"r") 
    data=fp.read().split()
    t=data[0::3]
    ...
    norm2=max(z)
    z=np.reshape(z,(tsize,xsize))/norm2
    ...
    # Set axis labels
    ax.set_xlabel(r'Location $2i$',fontsize=30)
    ax.set_ylabel(r'Time $t$',fontsize=30)
    ax.tick_params(axis='x', labelsize=30)
    ax.tick_params(axis='y', labelsize=30)
    ax.set_ylim(ax.get_ylim()[::-1])
    # Add a colorbar
    cbar = fig.colorbar(ScalarMappable(norm=norm, cmap=usemap), ax=ax)
    cbar.set_label(r"$|C_{ij}(t)|$",fontsize=30)
    plt.tight_layout()
...
def fitting_quench():
    data=open("./thermalize_quantum_quenchCNOT.txt","r")
    ...
    y=np.array([complex(i) for i in data[1::4]])
    y=np.log10(y)
...
def plot_heatmap_correlation3(filename):
    ...
    cbar.set_label(r"$\log_{10}|C_{ij}(t)|$",fontsize=30)
    plt.tight_layout()
    plt.savefig("/home/xiehangyu/yxh/study/mpq/quantum_circuit/script/Figs/"+filename.replace(".txt",".pdf"),dpi=300)
    # Show the plot
    plt.show()
