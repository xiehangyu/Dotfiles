...
def eigenvalue_for_measurement_total(t,U,p,write=1):
    ans= np.linalg.eigvals(channel_for_measurement_total(t,U,p))
    ans=sorted(ans,reverse=True)
    if write!=1:
        return ans
    with open('./plot_data/eigenvalue_for_measurement_total{}and{}.txt'.format(t,p),'w') as f:
        np.savetxt(f,U)
        np.savetxt(f,ans)
    return ans

def construct_eigenvalue_1(t):
    vec=np.array([1,0,0,1],dtype=complex)
    vec=np.kron(vec,vec)
    vec_ori=np.eye(1)
    for i in range(t+1):
        vec_ori=np.kron(vec_ori,vec)
    return vec_ori/np.linalg.norm(vec_ori)


def construct_eigenvalue_2(t):
...
def average_gap(t,p):
    ...
    for i in range(sample):
        ...
        ans=eigenvalue_for_measurement_total(t,U,p,write=0)
        if p==0:
            if abs(ans[1]-0.25)>1E-5:
                with open('./plot_data/exception_point.txt','a') as f:
                    np.savetxt(f,U)
                i=i-1
                continue
            if abs(ans[2]-0.25)<1E-5:
                with open('./plot_data/exception_point.txt','a') as f:
                    np.savetxt(f,U)
                i=i-1
                continue
            energy_gap += ans[1]-ans[2]
        if p!=0:
            if abs(ans[0]-0.25)>1E-5:
                with open('./plot_data/exception_point.txt','a') as f:
                    np.savetxt(f,U)
                i=i-1
                continue
            if abs(ans[1]-0.25)<1E-5:
                ...
                i=i-1
                continue
            energy_gap += ans[0]-ans[1]
    return energy_gap/sample

def multiple_average_gap(t):
    ps=[i*0.01 for i in range(0,21)]
    for p in ps:
        ans=average_gap(t,p)
        with open('./plot_data/average_gap{}.txt'.format(t),'a') as f:
            f.write('{}\t{}\n'.format(p,ans))
...
def random_dual_unitary(t,p):
    ...
    u1=unitary_group.rvs(2)
    u2=unitary_group.rvs(2)
    ans=eigenvalue_for_measurement_total(t,np.kron(u1,u2).dot(intm),p)
    return ans