...
def constructmethod2(D):
    ps=np.random.random([D])
    c_array=np.zeros([2,D,D],dtype=np.complex128)
    for i in range(D):
        c_array[0,i,i]=np.sqrt(ps[i])
        c_array[1,i,i]=np.sqrt(1-ps[i])
    total_check(c_array,D,2)
    total_check_dimension(c_array,D,2)
    print(dimension2(D))