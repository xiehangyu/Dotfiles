...
void daxsbxpxy_unroll2(double *X, double *Y, double alpha, double beta, const int N)
{
    ...
    for (i = 0; i < upperbound; i+=4)
    {
        ...
        temp3=Y[i];
        temp4=Y[i+1];
        Y[i] = temp1* (temp1*alpha + beta  + temp3);
        Y[i+1] = temp2* (temp2*alpha + beta  +  temp4);
        temp1=X[i+2];
        temp2=X[i+3];
        temp3=Y[i+2];
        temp4=Y[i+3];
        Y[i+2] = temp1 *(temp1*alpha + beta  +  temp3);
        Y[i+3] = temp2 *(temp2*alpha + beta  +  temp4);
    }
    for(;i<N;i++)//Here we deal with the rest of the loop
    {
        Y[i] = X[i] *(X[i]*alpha + beta  +  Y[i]);
...
{
    double new3,new2,new0,new1;
    new1=Y[1];
    new2=Y[2];
    int upperbound=(N-2)/4*4;
    for(int i=1;i<upperbound;i+=4)
    {
        new3=Y[i+2];
...
void simpletestfunction(double *X, double *Y, double alpha,double beta, const int N)
{
    ...
    daxsbxpxy(Xorigin,Yorigin,alpha,beta,N);
    daxsbxpxy_unroll2(Xtest,Ytest,alpha,beta,N);
    totaldifference=0;
    for (int i = 0; i < N; i++)
    {
        totaldifference+=(Yorigin[i]-Ytest[i])* (Yorigin[i]-Ytest[i]);
