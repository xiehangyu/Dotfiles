...
for repl1 in replacements["NMRURP"]:
    for repl2 in replacements["assoc"]:
        replaced_cmd = re.sub(r"--l1d_repl=\w+", f"--l1d_repl={repl1}", cmd)
        replaced_cmd = re.sub(r"--l1d_assoc=\d+", f"--l1d_assoc={repl2}", replaced_cmd)
        print(replaced_cmd)
        os.system(replaced_cmd)
        command = "cp -r m5out /home/xiehangyu/yxh/study/USTC/mm/computerarchtecturehomework/HW1/HW3_2" + repl1+"_"+repl2
        os.system(command)
