...
def test_phase_transition2Dplane(coefs):
    ...
    for coef in coefs:
        ...
        m1ss=np.sort(np.abs(np.linalg.eigvals(diagonal_symmetry(W,Nv))))[::-1]
        m2ss=np.sort(np.abs(np.linalg.eigvals(diagonal_asymmetry(W,Nv))))[::-1]
        m3ss=np.sort(np.abs(np.linalg.eigvals(offdiagonal_symmetry(W,Nv))))[::-1]
        m4ss=np.sort(np.abs(np.linalg.eigvals(offdiagonal_asymmetry(W,Nv))))[::-1]
        m1=m1ss[0]
        m2=m2ss[0]
        m3=m3ss[0]
        m4=m4ss[0]
