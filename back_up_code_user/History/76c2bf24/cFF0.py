...
#All the thing calculated from 0
...
def reduced_calculate_correlators(W,positions,op1=None,op2=None):
    ...
    print("The error is {}".format(np.abs(expect1-expectation_value(foldedchannel(C_array,16,2),fold_O1,0,2))))
    correlators=[]
    for x,y in positions:
        print("({},{})".format(x,y))
        expect2=reduced_W_expect(W,fold_O2,y)
        print("The expectation value 2 is {}".format(expect2))
        print("The error is {}".format(np.abs(expect2-expectation_value(foldedchannel(C_array,16,2),fold_O2,y,2))))
        cor=reduced_correlator(fold_O1,fold_O2,x,y,W)
        print("The correlator is {}".format(cor))
        print("The error is {}".format(np.abs(cor-MPS_direct_contraction(fold_O1,fold_O2,x,y,foldedchannel(C_array,16,2),2))))
...
def drawing():
    ...
    W=create_W_matrix3(0.3,0.3)
    positions=[]
    ilist=[]
    for i in range(3,20):
        positions.append((3,i))
        ilist.append(i)
    correlators=reduced_calculate_correlators(W,positions)
    print(correlators)
