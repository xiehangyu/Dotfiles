...
def temp5():
    ...
    for h in [0.01,0.02,0.04,0.08,0.12,0.16]:
        filename='NewOBCEntropyresultN{}alpha{:.6f}t{:0.6f}tend{:.6f}GSE{}Korder{}randomin{:.6f}J{:.6f}.txt'.format(N,alpha,t,tend,GSE,Korder,randomin,h)
        print(filename)
        #filename="XXexactN_{}t_{}tend_{}h_{}.txt".format(N,t,tend,h)
        fp=open(filename,'r')
        data=fp.read().split()
...
def temp6():
    ...
    for alpha in [0.0, 0.4]:
        ...
        plt.legend()
        plt.title(r"$\alpha={},h={}$".format(alpha,h))
        plt.savefig("/home/xiehangyu/yxh/study/mpq/Long_range_area_law/script/figs/alpha{}h{}.png".format(alpha,h))
        plt.show()
