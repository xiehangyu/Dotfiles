...
def reduced_calculate_correlators(W,positions,op1=None,op2=None):
    ...
    expect1=reduced_W_expect(W,fold_O1,2)
    print("The expectation value 1 is {}".format(expect1))
    print("The error is {}".format(np.abs(expect1-expectation_value(foldedchannel(C_array,16,2),fold_O1,2,2))))
    correlators=[]
    for x,y in positions:
        print("({},{})".format(x,y))
        expect2=reduced_W_expect(W,fold_O2,0)
        print("The expectation value 2 is {}".format(expect2))
        print("The error is {}".format(np.abs(expect2-expectation_value(foldedchannel(C_array,16,2),fold_O2,0,2))))
