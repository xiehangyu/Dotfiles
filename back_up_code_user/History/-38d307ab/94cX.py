...
def drawing_in_one_picture(N,J,tend):
    ...
    for alpha in {0,0.1}:
        ...
        plt.plot(times,entanglements,legend='OBC{}'.format(alpha))
        H=simple_longrange_Hamiltonian(N,alpha,J)
        psi0=tensor([basis(2,0)]*N)
        result=mesolve(H,psi0,ts,[],[])
        times=result.times
        ...
        for i in range(len(states)):
            entanglements.append(entropy_vn(states[i].ptrace([j for j in range(N//2)])))
        entropyss.append(entanglements)
        plt.plot(times,entanglements,legend='PBC{}'.format(alpha))
    ...
    for i in range(len(states)):
        entanglements.append(entropy_vn(states[i].ptrace([j for j in range(N//2)])))
    entropyss.append(entanglements)
    plt.plot(times,entanglements,legend='NNPBC')
    ...
    for i in range(len(states)):
        entanglements.append(entropy_vn(states[i].ptrace([j for j in range(N//2)])))
    entropyss.append(entanglements)
    plt.plot(times,entanglements,legend='NNOBC')
    plt.xlabel('t')
