...
Here we provide further analytical insights into the latter, focusing on both the kinematic and dynamical aspects. First, we unveil the underlying canonical typicality and atypicality for random free-fermion states%, which
...
on tensor networks \citep{RevModPhys.93.045003}, %and
while their violations may signature %entanglement also characterizes 
...
to the new theoretical insights from quantum thermalization \citep{PhysRevLett.115.267206,PhysRevLett.119.220603,Nakagawa2018,PageCurve_Thermal,Lu2019,PageCurve_Thermal2,PhysRevLett.125.021601} and %PageCurve_Thermal3},
...
This typicality %\textcolor{blue}{kinematic} 
behavior
...
very recently \citep{Bianchi2021,Bianchi2021a}. It turns out to be similar to the interacting case for a small subsystem, but differ significantly otherwise. See Fig.~\ref{Setup_of_CCRFG_ensemble}(b) for an illustration. We establish the corresponding %a qualitatively weaker 
canonical typicality and atypicality that explicitly explains the similarity and difference. In addition, we show that the free-fermion Page curve can be relevant to extremely simple tight-binding models via long-time quench dynamics. %although it still obeys the volume law. Here comes a natural question: ? 
...
\textcolor{red}{EE is} an important macroscopic quantity
%is entropy, 
...
to indicate thermalization. %The EE also relates to phase transition in condensed matter physics \citep{relation_entropy_Phase}.
...
The study of non local EE becomes possible, which
...
consider number-conserving systems with totally $N$ modes occupied by $N/2$ fermions, i.e., the half-filling case. Compared to the fully random case without number conservation, this setting appears to be more physically comprehensible and experimentally relevant, while displaying exactly the same Page curve %half filling case, because it has been shown this case dominates the contribution of Page curve for RFG-ensemble. The volume-law coefficient of half-filling case also converges to the latter one
...
free-fermion system has $N$ sites with half filling. The %physical 
subsystem of interest %we are interested in 
has $N_{A}$ ($N_{A}\le%<
N$) sites. %shows the states are in the 
The RFG ensemble is generated by Haar-random Gaussian unitaries with number conservation. %induced by Haar measure over $\mathbb{\textcolor{red}{U}}(N)$.
(b) %shows the state is evolved with some hopping terms.}
The Page curves of the RFG %-ensemble 
and interacting ensemble in the thermodynamic limit $N\to\infty$. %From the figure, 
It is obvious that these two Page curves agree with each other in the microscopic region but show a $\mathcal{O}(1)$ deviation in the macroscopic region. The interacting Page curve in the thermal dynamical limit is always saturated. (c) The table summarizes the typicality/atypicality property for the RFG and interacting ensembles. Here ``Poly." and ``Exp." indicates polynomial and exponential scalings, respectively.}
...
An important property of Gaussian states is that their subsystems remain %will also be 
Gaussian. %and thus fully characterized by the covariance matrix. 
...
\begin{equation}
\mathbb{P}(S_{A}\geq N_{A}-\xi_{a}+%\frac{2}{\ln2}
\epsilon) & \leq2e^{-\frac{\epsilon^{2}}{\xi'_{a}}},\;\;\;\;
\forall\epsilon>0
\label{eq:main_text_atypicality_for_entropy}
\end{equation}
in the macroscopic region. Here $\xi=\sqrt{\frac{2N_{A}^{2}}{N-1}}$, $\xi'=\frac{192}{N}$, $\xi_{a}=\frac{N_{A}^{2}}{2\ln2(N+1)}$ and $\xi'_{a}=\frac{192N_{A}}{\ln^22}N$. 
...
10^{-3}$ which agrees with our analysis. A relatively larger deviation of our theoretical result from the other two curves at $f=\frac{1}{2}$ is due to the truncation. This figure can also represent the general dynamical Page curve for Hamiltonians in (a). %argument. 
...
To simplify the analytic calculations, we assume the Hamiltonian $H$ to be translation-invariant (under the periodic boundary condition) and specify our initial state $|\Psi_0\rangle$ to be %is 
...
%What's more, we also 
and pointed out the quatitative %\textcolor{red}{the qualitative} %that there is obvious 
...
additional symmetries (such as the Altland-Zirnbauer symmetries \cite{Altland1997}). %to the Hamiltonians.
