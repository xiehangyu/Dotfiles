...
auto LongrangeHamiltonian(double alpha)
...
void timeevolutionTDVP(double alpha, double t=0.1, double tend=0.5, int n_GSE=3, int Korder=3, double randomin=0)
{//Time evolution using TDVP, the decay exponent of the interaction is alpha, the time step is t, the total time is tend, the number of global subspace expansion is n_GSE, the order of Krylov subspace is Korder, the two printed results are the deviation of the expectation value of the total spin from its maximum value and the half chain entropy.
    ofstream fp;
    fp.open("./data/EntropyresultN"+to_string(N)+"alpha"+to_string(alpha)+"t"+to_string(t)+"tend"+to_string(tend)+"GSE"+to_string(n_GSE)+"Korder"+to_string(Korder)+"randomin"+to_string(randomin)+".txt");
    auto psi1=productstate();
    auto H=LongrangeHamiltonian(alpha);
...
//addBasis(psi1,H,epsilonK,{"Cutoff",1E-9,"Method","DensityMatrix","KrylovOrd",Korder,"DoNormalize",true,"NumCenter",1});
 addBasis(psi1,H,epsilonK,{"Cutoff",1E-9,"Method","DensityMatrix","KrylovOrd",Korder,"DoNormalize",true});
            }
        
        // TDVP sweep
        
   //     tdvp(psi1,H,-t*1_i,sweeps,{"Truncate",true,"DoNormalize",true,"Quiet",true,"ErrGoal",1E-9,"IsHermitian",true,"NumCenter",2});
        ...
        fp<<t*n<<" "<<N/2*(N/2+1)-real(innerC(psi1,observable,psi1))<<endl;
        fp<<t*n<<" "<<vn_entropy(psi1,N/2)<<endl;
        }
...
{//The input parameters are alpha, number of sites, time step, total time, number of global subspace expansion, order of Krylov subspace
    ...
    {//Use the input value from the user
        ...
        timeevolutionTDVP(alpha,t,tend,GSE,Korder);
    }
    return 0;
}