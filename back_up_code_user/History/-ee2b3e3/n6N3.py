...
def compared_with_PRRxx():
    ...
    for h in [0.2,0.4,0.5,0.8]:
        ...
        if h==0.3:
            filename="XXspinwave_N{}_alpha{}_t{}_tend{}_h{}.txt".format(50,0.0,t,60.0,h)
            fp=open(filename,'r')
            data=fp.read().split()
            ts=np.array(data[0::2],dtype=float)
            Entropys=np.array(data[1::2],dtype=float)
...
def compared_with_PRR():
    ...
    for h in [0.2]:
        ...
        plt.legend()
        plt.title(r"$\alpha={},h={}$".format(alpha,h))
        plt.savefig("/home/xiehangyu/yxh/study/mpq/Long_range_area_law/script/figs/exactcompared_a_alpha{}h{}.png".format(alpha,h))
        plt.show()
    t=0.02
    tend=60
    Korder=3
    ...
    for h in[0.2,0.8]:
        ...
        plt.legend()
        plt.title(r'$\alpha={},h={}$'.format(alpha,h*2))
        plt.savefig("/home/xiehangyu/yxh/study/mpq/Long_range_area_law/script/figs/compared_c_alpha{}h{}.png".format(alpha,h*2))
        plt.show()
    alpha=0.7
    h=0.2
    GSE=22
    for h in [0.2,0.8]:
        ...
        plt.legend()
        plt.title(r'$\alpha={},h={}$'.format(alpha,h*2))
        plt.savefig("/home/xiehangyu/yxh/study/mpq/Long_range_area_law/script/figs/compared_d_alpha{}h{}.png".format(alpha,h*2))
        plt.show()
