...
def Naive_generation_go_through(D,chi):
    U=unitary_group.rvs(D*chi).reshape([D,chi,chi,D])
    M=(np.random.random([D*chi,D*chi])+1j*np.random.random([D*chi,D*chi])).reshape([D,chi,chi,D])
    state=np.array([1]+[0]*(D-1))
    return ncon([U,M,state],[[-1,-5,-3,1],[1,-4,-2,2],[2]])

def test_Naive_generation_go_through(D,chi):
    Check_gothrough_condition(Naive_generation_go_through(D,chi),D)
...
def go_through_tangent_space_naive_case(D,chi):
    C_array=Naive_generation_go_through(D,chi)
    return(calculate_tangent_space_dimension_go_through(C_array,D=D,chi=chi))
...
def go_through_minimization_costfunction(vars,D,chi):
    vars_temp=vars.reshape([2,D,chi,chi,chi,chi])
    vars_tensor=vars_temp[0]+1j*vars_temp[1]
    vars_tensor=vars_tensor/np.linalg.norm(vars_tensor)
    return np.sqrt(Check_gothrough_condition(vars_tensor,chi))
...
def Go_through_guess_by_minimization(D,chi):
    ...
    C_array_temp=result.x.reshape([2,D,chi,chi,chi,chi])
    C_array=C_array_temp[0]+1j*C_array_temp[1]
    return calculate_tangent_space_dimension_go_through(C_array,D,chi),C_array
