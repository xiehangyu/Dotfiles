...
def project_to_dual(vectorized_channel,chi):
    ...
    if abs(coef)<1e-8:
        ...
        project_channel=project_channel.reshape([chi**2]*4)/coef
        matrix_c=project_channel.reshape([chi]*8).transpose([0,2,4,6,1,3,5,7]).reshape([chi**4]*2)
        eigenvalues=LA.eigvalsh(matrix_c)
        print(eigenvalues)
        if eigenvalues[0]<0:
            matrix_c =(matrix_c-np.eye(chi**4)*eigenvalues[0])/(1-eigenvalues[0]*chi**2)
        project_channel=matrix_c.reshape([chi]*8).transpose([0,4,1,5,2,6,3,7]).reshape([chi**2]*4)
...
def another_solvable_PEPS_fold(D,chi):
    vectorized_channel=vectorizedchannel(random_PEPS(D,chi),D,chi)
...
def random_dual_PEPS(D,chi):
    while True:
        ...
        if D>=chi**4 or (eigenvalues[-D-1]/eigenvalues[-1])<=1e-7:
            C_array=np.diag(np.sqrt(eigenvalues[-D:]))@(eigenvectors[:,-D:].conj().T)
            C_array=C_array.reshape([D,chi,chi,chi,chi])
            check_condition(C_array,D,chi)
            return C_array
        else:
            print(eigenvalues[-D-1]/eigenvalues[-1])
