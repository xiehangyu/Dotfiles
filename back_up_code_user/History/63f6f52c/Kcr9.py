...
def singledecaytimecf():
    ...
    h=np.random.rand(6,6)+1j*np.random.rand(6,6)
    h=(h+h.conj().T)/2
    tr=np.trace(h)
    h=h-tr*np.eye(6)/6
    normalizationfactor=np.trace(h.conj().T.dot(h))
    h=h/np.sqrt(normalizationfactor)
    ts=[]
    xs=[]
    fp=open("./plot_data/trialfittingdata.txt",'w')
    for t in range(30,80,2):
        ts.append(t)
        tempans=time_correlation_function(t,0,h,h,U,U)
        print("{}\t{}\n".format(t,tempans))
        xs.append(np.abs(tempans))
        fp.write("{}\t{}\n".format(t,np.abs(tempans)))
    plt.plot(ts,xs)
    plt.show()
    xs=np.log(xs)
    plt.plot(ts,xs)
...
def create_picture_datasupport2(filename):
    U=generate_matrix(6,0,3,0)
    X=np.array([[0,1],[1,0]])
    U1=np.kron(np.eye(2),unitary_group.rvs(3))
    U2=np.kron(np.eye(2),unitary_group.rvs(3))
    U1=np.kron(np.eye(2),np.diagonal([1,-1,1]))
    U2=np.kron(np.eye(2),np.diagonal([1,-1,1]))
    U=np.kron(U1,U2).dot(U)
    a,b=generalwhether_left_invariant(U)
    print("the left deviation is {}".format(np.trace(b.conj().T.dot(b))))
    a,b=generalwhether_right_invariant(U)
