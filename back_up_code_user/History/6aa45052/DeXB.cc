...
auto LongrangeAntiHeisenberg(double alpha, double h=0.0)
{
    ...
    for(int i=1;i<=N;i++)
    {
        for(int j=i+1;j<=N;j++)
        {
            ampo+=0.2*normfactor/pow(1.0+Period_length(i,j),alpha),"Sx",i,"Sx",j;
            ampo+=0.5*normfactor/pow(1.0+Period_length(i,j),alpha),"Sy",i,"Sy",j;
...
void imaginarytimeevolutionTDVP(double alpha, double t=0.1, double tend=20, int n_GSE=3, int Korder=3, double h=0)
{
    ...
    for(int k=1;k<=N-1;k++)
    {
        ...
        auto YY=correlators2point(0,k,"Sy","Sy",psi);
        auto ZZ=correlators2point(0,k,"Sz","Sz",psi);
