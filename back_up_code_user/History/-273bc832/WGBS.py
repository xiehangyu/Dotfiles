...
def construct_phis(M):
    ...
    print(len(return_list),flush=True)
    return return_list
...
def test_a_class_of_Dual_MT():
    ...
    temp_op=kron_product([Op_h@X@Op_hI,Op_v@X@Op_vI,Op_h@X@Op_hI,Op_v@X@Op_vI])
    eigenvalues,eigenvectors=np.linalg.eig(temp_op)
    print(eigenvalues)
    positive_eigenvalues=eigenvalues > 0.9
    positive_eigenvectors=eigenvectors[:,positive_eigenvalues]
    Op_last=positive_eigenvectors@np.conj(positive_eigenvectors).T
    negative_eigenvectors=scipy.linalg.null_space(Op_last.T)
    Op_last = Op_last-negative_eigenvectors@np.conj(negative_eigenvectors).T
    extended_op=ncon([I,I,I,I,I,I,I,I,Op_last.reshape([2]*8)],[[-1,-13],[-2,-14],[-4,-16],[-5,-17],[-6,-18],[-9,-21],[-11,-23],[-12,-24],[-3,-7,-8,-10,-15,-19,-20,-22]]).reshape([2**12,2**12])
    Oas.append(extended_op)
    ...
    projection_to_orthogonal=np.conj(orthogonal_matrix).T.dot(finding_op).dot(orthogonal_matrix)
    print(scipy.linalg.eigvalsh(projection_to_orthogonal),flush=True)
