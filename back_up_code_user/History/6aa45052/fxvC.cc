...
void imaginarytimeevolutionTDVP(double alpha, double t=0.1, double tend=20, int n_GSE=3, int Korder=3, double h=0)
{
    ...
    for(int n=1;n<=nsw;n++)
    {
        ...
        energy=tdvp(psi,H,-t,sweeps,{"Truncate",true,"DoNormalize",true,"Quiet",true,"ErrGoal",1E-8,"NumCenter",2});
        psi.normalize();
    ...
    for(int k=1;k<=N-1;k++)
    {
        fp<<k<<"  "<<correlators2point(0,k,"Sx","Sx",psi)<<"  "<<correlators2point(0,k,"Sy","Sy",psi)<<"  "<<correlators2point(0,k,"Sz","Sz",psi)<<endl;
    }
    fp.close();
}
