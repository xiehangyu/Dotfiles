...
def newnormfactor(N,alpha):
    ...
    return normfactor
def simple_longrange_Hamiltonian_SB(N,alpha,J=0.0):
    Z=sigmaz()/2
    X=sigmax()/2
    ...
    return_H = return_H/newnormfactor(N,alpha)*2
    for i in range(N):
        return_H += J*tensor([identity(2)]*i+[X]+[identity(2)]*(N-i-1))
    return return_H
def NearestNeighbor_SB(N,J=0.0):
    X=sigmax()
    Z=sigmaz()
    returnop=Qobj(dims=[[2]*N,[2]*N])
    for i in range(N-1):
        returnop += tensor([identity(2)]*i+[Z]+[Z]+[identity(2)]*(N-i-2))
    returnop += tensor([Z]+[identity(2)]*(N-2)+[Z])
    for i in range(N):
        returnop += J*tensor([identity(2)]*i+[X]+[identity(2)]*(N-i-1))
    return returnop

def dynamical_phase_transition(h):
    N=12
    H2=NearestNeighbor_SB(N,h)
    H=simple_longrange_Hamiltonian_SB(N,0.1,h)
    psi0=tensor([basis(2,0)]*N)
