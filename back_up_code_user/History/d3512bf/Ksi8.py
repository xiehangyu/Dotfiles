...
def HT_Plot(x,y,colormap='hsv',figname="HT_AVLog1.pdf",T=None):
    ...
    '''
    在yscale中的symlog其实并不一直是按照对数坐标画图的。在靠近原点的附近(-linthresh,linthresh)是按照线性坐标画图的。linscale参数就指定了这段线性区间(0,linthresh)在图上的长度相对于对数坐标单个区间（例如区间10-100）的比例。如果linthresh与linscale设置的值不匹配， 就会使得坐标区间看起来不是均匀分布的。在python的default value中，这两者的设置是不匹配的。
    '''
