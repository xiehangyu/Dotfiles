...
def Gaussian_evolution(h,gamma,timeslides,N=None):
    ...
    for i in range(len(timeslides)):
        if not results.states[i].isoper:
            print("wrong")
            exit()
        results_matrix.append(results.states[i].unit().full()*N)
    return results_matrix


def entropy_calculation(rho):
    rho=(rho+np.conj(rho).T)/2.0
    rho=rho/np.trace(rho)*(np.shape(rho)[0]//2)
    eigenvalues=scipy.linalg.eigvalsh(rho)
    entropy=0
    cutoff=1E-7
    for eigenvalue in eigenvalues:
        if (eigenvalue <=cutoff) or (eigenvalue >=cutoff):
            continue
        else:
...
def simple_initial_state_entropy_evolution(N,alpha,J,total_time,dt=0.2):
    ...
    for i in range(len(result_gammas)):
        entropys.append(entropy_calculation(subsystemgamma(result_gammas[i],N//2,N)))
