...
double determinantofmatrix(vector<vector<double>> A)
{
...
// matrix, Q is the matrix of eigenvectors, and eigenvalue is the vector of eigenvalues.
    ...
    if(whethertest==true)
    {
        auto tempvec=eigenvalue;
        sort(tempvec.begin(),tempvec.end());
        cout<<"The eigenvalues of AAT are:"<<endl;
        for(int i=0;i<=n-1;i++)
        {
            cout<<tempvec[i]<<endl;
        }
        cout<<endl;
        cout<<"Using determinant to test the eigenvalues for AAT:"<<endl;
        for(int i=0;i<tempvec.size();i++)
        {
            auto tempmatrix=M;
            for(int j=0;j<=n-1;j++)
            {
                tempmatrix[j][j]-=tempvec[i];
            }
        
            double det=determinantofmatrix(tempmatrix);
            cout<<"We test the eigenvalue "<<tempvec[i]<<endl;
            cout<<"The determinant of AAT-eigenvalue*I is "<<det<<endl;
        }
    }
}
