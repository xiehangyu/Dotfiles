...
def FigureC():
    ...
    fp3=open('.fit/Falling_400nm_vacuum_Origin.txt','w')
    fp4=open(".fit/Falling_400nm_vacuum_Fitting.txt",'w')
    fp5=open('.fit/Falling_400nm_air_Origin.txt','w')
    fp6=open('.fit/Falling_400nm_air_Fitting.txt','w')
    ...
    xs2=np.linspace(xmin2-0.007,xmax2,500)
    yfit1=[]
    for i in xs1:
    ...
    plt.ylabel(r"Current $(\times 10^{-2}nA)$",fontsize=25)
    for i in range(len(xdraw1)):
        fp3.write(str(xdraw1[i])+" "+str(ydraw1[i])+"\n")
    for i in range(len(xdraw2)):
        fp5.write(str(xdraw2[i])+" "+str(ydraw2[i])+"\n")
    for i in range(len(xs1)):
        fp4.write(str(xs1[i])+" "+str(yfit1[i])+"\n")
    for i in range(len(xs2)):
        fp6.write(str(xs2[i])+" "+str(yfit2[i])+"\n")
    ...
    plt.legend(["Fit Vacuum","Fit Air","Experiment Vacuum","Experiment Air"],fontsize=30,bbox_to_anchor=(0.98,0.61))
    plt.tight_layout()
...
def FigureD():
    ...
    plt.scatter(xdraw1,ydraw1,s=150,marker='o', facecolors='none', edgecolors='red')
    plt.scatter(xdraw2,ydraw2,s=150,marker='o', facecolors='none', edgecolors='blue')
    fp3=open('.fit/Rising_400nm_vacuum_Origin.txt','w')
    fp4=open('.fit/Rising_400nm_vacuum_Fitting.txt','w')
    fp5=open('.fit/Rising_400nm_air_Origin.txt','w')
    fp6=open('.fit/Rising_400nm_air_Fitting.txt','w')
    for i in range(len(xdraw1)):
        fp3.write(str(xdraw1[i])+" "+str(ydraw1[i])+"\n")
    for i in range(len(xdraw2)):
        fp5.write(str(xdraw2[i])+" "+str(ydraw2[i])+"\n")
...
def FigureB():
    ...
    plt.tick_params(axis='x', direction='in')
    plt.tick_params(axis='y', direction='in')
    plt.text(205-temp,30, 'Air', ha='center', va='bottom',fontsize=20)
    plt.text(199-temp,10, 'Vacuum', ha='center', va='top',fontsize=20)
    plt.tight_layout()
