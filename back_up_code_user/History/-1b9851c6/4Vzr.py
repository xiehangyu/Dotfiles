...
def LongrangeHamiltonian(N,alpha):
    ...
    for i in range(N):
        for j in range(i+1,N):
            returnop+=1.0*tensor([identity(2)]*i+[X]+[identity(2)]*(j-i-1)+[X]+[identity(2)]*(N-j-1))/normfactor/(PD(i,j,N)**alpha)
            returnop+=1.0*tensor([identity(2)]*i+[Y]+[identity(2)]*(j-i-1)+[Y]+[identity(2)]*(N-j-1))/normfactor/(PD(i,j,N)**alpha)
            returnop+=1.0*tensor([identity(2)]*i+[Z]+[identity(2)]*(j-i-1)+[Z]+[identity(2)]*(N-j-1))/normfactor/(PD(i,j,N)**alpha)
    return returnop
...
def different_alpha_correlators(N):
    for alpha in range(21):
        ...
        sum_energy=0
        for i in range(1,N):
            sum_energy+=0.5*((-1)**(i))*(N**(alpha/100.0))/(PD(i,0,N)**(alpha/100.0))
        print("The naive ground state energy is {}".format(sum_energy))
        psi=groundstate(LongrangeHamiltonian(N,alpha/100.0))
