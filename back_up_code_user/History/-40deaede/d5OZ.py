...
def channel_for_measurement_total(t,U,p):
    ...
    initialstate=np.kron(initialstate,np.eye(16**(t-1)))
    ...
    Z=np.kron(Z,Z)
    Y=np.kron(Y,Y)
    I=np.kron(np.eye(16**(t-1)),I)
    X=np.kron(np.eye(16**(t-1)),X)
    Y=np.kron(np.eye(16**(t-1)),Y)
    Z=np.kron(np.eye(16**(t-1)),Z)
...
def eigenvalue_for_measurement_total(t,U,p):
    return np.linalg.eigvals(channel_for_measurement_total(t,U,p))
