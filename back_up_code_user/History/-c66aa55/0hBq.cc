...
void daxsbxpxy_unroll2(double *X,double *Y, double alpha, double beta, const int N)
{
    ...
    for(i=0;i<upperbound;i+=4)
    {
        temp[1]=alpha*X[i]*X[i];
        temp[2]=alpha*X[i+1]*X[i+1];
        temp[3]=alpha*X[i+3]*X[i+3];
        temp[0]=alpha*X[i+2]*X[i+2];
        Y[i] =beta * X[i] + X[i] * Y[i]+temp[1];
        Y[i+1]= beta * X[i+1] + X[i+1] * Y[i+1]+temp[2];
        Y[i+2]= beta * X[i+2] + X[i+2] * Y[i+2]+temp[0];
        Y[i+3]= beta * X[i+3] + X[i+3] * Y[i+3]+temp[3];
    }
    for(;i<N;i++)//Here we deal with the rest of the loop
    {
        Y[i] =alpha* X[i] * X[i]+ beta * X[i] + X[i] * Y[i];
...
{
    //try to use software pipelining to improve the performance. But the result is not good.
    double new3,new4;
    new3=Y[2];
    int upperbound=(N-1)/4*4;
    int i;
...
int main()
{
    ...
    m5_dump_reset_stats(0, 0);
    daxsbxpxy_unroll2(X, Y, alpha, beta, N);
    m5_dump_reset_stats(0, 0);
    stencil(Y, alpha, N);
    m5_dump_reset_stats(0, 0);
