...
def draw_picture_comparison_gaussian_interaction():
    ...
    plt.tight_layout()
    plt.savefig("../script/figs/illustration_of_mi_or_ma.pdf")

def draw_picture_finite_system_size():
    fs=[0]
    entropy1=[0]
    entropy2=[0]
    instance=trun_k_probability(20,10,5)
    for i in range(1,20):
        fs.append(i/20)
        instance.change_parameter(20,10,i)
        entropy1.append(instance.direct_entropy_without_sum()/20)
        instance.change_parameter(20,5,i)
        entropy2.append(instance.direct_entropy_without_sum()/20)
    fs.append(1)
    entropy1.append(0)
    entropy2.append(0)
    plt.figure(figsize=[12,8],dpi=300)
    mpl.rcParams["mathtext.fontset"] = "stix"
    mpl.rcParams['font.family'] = 'STIXGeneral' 
    plt.plot(fs,entropy1)
    plt.plot(fs,entropy2)
    plt.xticks(fontsize=40)
    plt.yticks(fontsize=40)
    plt.xlabel(r"$f=N_A/N$",fontsize=40)
    plt.ylabel(r"$\frac{\langle{S_A}\rangle}{N}$",fontsize=40)
    plt.legend(["Charge Number=10","Charge Number=5"],prop={'size':30})
    plt.tight_layout()
    plt.savefig("../script/figs/finite_system_size_CRFG_different_charge_number.pdf")
    plt.show()
    fs=[0]
    entropy1=[0]
    entropy2=[0]
    for i in range(1,20):
        fs.append(i/20)
        instance.change_parameter(20,i,10)
        entropy1.append(instance.direct_entropy_without_sum()/20)
        instance.change_parameter(20,i,5)
        entropy2.append(instance.direct_entropy_without_sum()/20)
    fs.append(1)
    entropy1.append(0)
    entropy2.append(0)
    ...
    plt.xticks(fontsize=40)
    plt.yticks(fontsize=40)
    plt.xlabel(r"$\frac{m}{N}$",fontsize=40)
    plt.ylabel(r"$\frac{\langle{S_A}\rangle}{N}$",fontsize=40)
    plt.legend(["Subsystem Size=10","Subsystem Size=5"],prop={'size':30}) 
    plt.tight_layout()
    plt.savefig("../script/figs/finite_system_size_CRFG_different_subsystem_size.pdf")
    plt.show()
    
def finite_effect_Gaussian():
    N=20
    N=40
    entropys1=[0]
    entropys2=[0]
    entropys3=[0]    
    fs=[0]
    temp=trun_k_probability(20,10,1)
    for i in range(1,20):
        temp.change_parameter(20,10,i)
        entropys1.append(temp.direct_entropy_without_sum()/20)
        temp.change_parameter(40,20,i*2)        
        entropys2.append(temp.direct_entropy_without_sum()/40)
        f=i/20
        fs.append(f)
        entropys3.append(Gaussian_entropy_thermal_dynamical_limit(f))
    fs.append(1)
    entropys1.append(0)
    entropys2.append(0)
    entropys3.append(0)
    plt.plot(fs,entropys1,fs,entropys2,fs,entropys3)
    plt.legend(["N=20","N=40","Gaussian"])
    plt.show()
...
if __name__=="__main__":
    ...
    finite_effect_Gaussian()
    #draw_picture_finite_system_size()