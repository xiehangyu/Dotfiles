...
def calculate_entropy_directly(NA,N):
   ...
   # return (temp1+temp2+temp3+temp4+temp5+temp6+temp7+temp8+temp9+temp10)/np.log(2)
    return (temp1+temp2+temp3+temp4+temp5)/np.log(2)
...
def read_from_Hamiltonian_evolution():
    ...
    fs=open("./plot_data/OnSitePotential_Hamiltonian_evolution_Nsite{}_Tperiod{}_Initialtime{}_Samplingtime{}Spacing{}{}.txt".format(N_sites,T_period,Initialtime,Samplingtime,space,fileindex),"r")
...
#       plt.legend(["Dynamical Page Curve","Random Gaussian State Page Curve","Taylor Expansion Result","Quasi Particle Picture"])
        ...
        plt.savefig("../script/figs/Onsite{}Taylor_detailed.pdf".format(fileindex))
        plt.show()
        plt.figure(figsize=[12,8],dpi=300)
        plt.plot(Calculate_list,np.array(value)/N_size,'b',marker='s')
        plt.plot(Calculate_list,np.array(entropy_array2)/N_size,'r',marker='d')
        ...
        plt.yticks(fontsize=40)
        plt.tight_layout()
        plt.legend(["Dynamical","Fully Random","Theory"],loc='upper right',prop={'size':25})
        plt.savefig("../script/figs/Onsite{}.png".format(fileindex))
        plt.show()
    if whether_random==0:
        ...
        plt.xticks(fontsize=40)
        plt.yticks(fontsize=40)
        plt.savefig("../script/figs/{}Taylor_detailed.pdf".format(fileindex))
        plt.show()
        plt.figure(figsize=[12,8],dpi=300)
        plt.plot(Calculate_list,np.array(value)/N_size,'b',marker='s')
        plt.plot(Calculate_list,np.array(entropy_array2)/N_size,'r',marker='d')
        ...
        plt.tight_layout()
        plt.legend(["Dynamical","Fully Random","Theory"],loc='upper right',prop={'size':25})
        plt.savefig("../script/figs/{}Taylor.pdf".format(fileindex))
        plt.show()
