...
def HT_Plot2(x, y, colormap='hsv', figname="HT_AVLog2.pdf", T=None):
   ...
   在一般的科研文献作图中，我建议fontsize至少取到30吧，我一般取到40（做PPT时也一样），不然可能会在文献里看不清楚。
    ...
    '''
    plt.legend([r"$"+str(i)+"K$" for i in T], fontsize=16, loc='upper left')
    '''
...
def HT_Plot3(x,y,colormap='hsv',figname="HT_AVLog1.pdf",T=None):
    plt.figure(figsize=(10,8))
    '''
    换了一个函数取colormap
    colors = mpl.cm.get_cmap(colormap, 15)
    ...
    num = len(x)
    for i in range(num):
        plt.plot(np.log(np.array(x[i])), np.log(np.array(y[i]))+i, c=c[i], linewidth=2)
    '''
    在log作图时，建议还是加上minor ticks（或称subticks）吧。它的意思是，除了显示例如1E-1,1E0,1E1的坐标外，还会在指定的子坐标上（例如2，3，4，）上显示刻线。我感觉这点还是挺有用的，可以帮助在log绘图上更轻易地读数。
    '''
    plt.minorticks_on()
...
if __name__ == '__main__':
    ...
    '''
    HT_Plot(x, y, colormap='turbo',figname="HT_AVLog1.pdf",T=T)
    HT_Plot2(x, y, colormap='turbo',figname="HT_AVLog2.pdf",T=T)
    HT_Plot3(x, y, colormap='turbo',figname="HT_AVLog3.pdf",T=T)
    '''
    加了一个T参数，用来写legend。
    '''
